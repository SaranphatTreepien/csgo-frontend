  ...
  <TouchableOpacity style={cs.card} onPress={onPress} activeOpacity={0.8}>
    ...
    <View style={cs.info}>
      ...
      <View style={[cs.rarityBadge, { borderColor: item.rarityColor }]}>
        <Text style={[cs.rarityText, { color: item.rarityColor }]}>{item.rarity}</Text>
      </View>
      <Text style={cs.price}>
        {item.listingPrice
      ...
      {item.sellerName && (
        <Text style={cs.seller}>👤 {item.sellerName}</Text>
      )}
    </View>
  </TouchableOpacity>
);
...
export default function StoreScreen({ navigation }) {
  ...
  const handleBuy = (item) => {
    navigation.navigate('Payment', { items: [{ ...item, price: item.listingPrice || item.basePrice }] });
  };
    ...
    <SafeAreaView style={s.safe} edges={['top']}>
      ...
      <View style={s.tabRow}>
        <TouchableOpacity
          ...
          onPress={() => { setActiveTab('database'); setPage(1); }}
        >
          <Text style={[s.tabText, activeTab === 'database' && s.tabTextActive]}>
            🎮 CS2 Items ({activeTab === 'database' ? total : '2092'})
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[s.tab, activeTab === 'listings' && s.tabActive]}
          onPress={() => { setActiveTab('listings'); setPage(1); }}
        >
          <Text style={[s.tabText, activeTab === 'listings' && s.tabTextActive]}>
      ...
      <FilterModal visible={filterVisible} onClose={() => setFilterVisible(false)} onApply={() => {}} />
    </SafeAreaView>
  );
}
...
const s = StyleSheet.create({
  ...
  catBtnActive: { backgroundColor: colors.primary + '22', borderColor: colors.primary },
...
const cs = StyleSheet.create({
  card: {
    flex: 1, margin: 4, backgroundColor: colors.cardBg,
    borderRadius: 10, overflow: 'hidden',
    borderWidth: 1, borderColor: colors.border,
  },
  rarityBar: { height: 3 },
  imageBox: {
    height: 110, backgroundColor: colors.surfaceElevated,
